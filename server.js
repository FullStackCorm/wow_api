...
app.get('/raid/:area', (req, res) => {
    ...
    //makes an array of every collection document
    collection.find().toArray().then(results => {
      //makes an object of the document in the collection that shares a name with the url (raidLocation)
      const areaObject = results.filter(obj => obj.areaName == raidLocation)[0];
      //renders the area with the corresponding object
      res.render('area.ejs', {area: areaObject})
    })
